...
interface GsacPhotoDoc extends WithId {
  imageUrl: string;
  storagePath: string;
  order: number;
}
...
/**
 * Photo gallery for the Graduate Study Abroad Center (GSAC) page — a flat,
 * ordered list of uploaded photos (unlike the per-office galleries in
 * Industry Liaison Office Photos, GSAC has just one gallery for the whole
 * page).
 */
export default function GsacPhotosAdmin() {
  const { docs: photos, loading } = useOrderedCollection<GsacPhotoDoc>('gsacPhotos', 'order');
  const { openCrop, cropModal } = useImageCropModal(4 / 3);
  const [uploading, setUploading] = useState(false);

  const addPhoto = (file: File) => {
    setUploading(true);
    openCrop(file, 'vwu/gsac', async (result: UploadResult) => {
      try {
        await addDoc(collection(db, 'gsacPhotos'), {
          imageUrl: result.url,
  ...
  const removePhoto = async (photo: GsacPhotoDoc) => {
    if (!confirm('Remove this photo?')) return;
    try {
      await deleteDoc(doc(db, 'gsacPhotos', photo.id));
  ...
  return (
    <div className="admin-card">
      <h2 className="admin-card__title">GSAC Photos ({photos.length})</h2>
      <p className="admin-lead" style={{ marginBottom: '1.5rem' }}>
        Upload the photos shown on the Graduate Study Abroad Center (GSAC) placement sub-page.
      </p>
