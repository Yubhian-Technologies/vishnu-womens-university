...
interface TiDspGalleryPhotoDoc extends WithId {
  imageUrl: string;
  storagePath: string;
  order: number;
}
...
/**
 * Photo gallery for the TI-DSP Centre of Excellence differentiator page —
 * a flat, ordered list of uploaded lab/event photos, same pattern as
 * TEDxSVECW Photos.
 */
export default function TiDspGalleryPhotosAdmin() {
  const { docs: photos, loading } = useOrderedCollection<TiDspGalleryPhotoDoc>('tiDspGalleryPhotos', 'order');
  const { openCrop, cropModal } = useImageCropModal(4 / 3);
  const [uploading, setUploading] = useState(false);

  const addPhoto = (file: File) => {
    setUploading(true);
    openCrop(file, 'vwu/tidspcoe', async (result: UploadResult) => {
      try {
        await addDoc(collection(db, 'tiDspGalleryPhotos'), {
          imageUrl: result.url,
  ...
  const removePhoto = async (photo: TiDspGalleryPhotoDoc) => {
    if (!confirm('Remove this photo?')) return;
    try {
      await deleteDoc(doc(db, 'tiDspGalleryPhotos', photo.id));
  ...
  return (
    <div className="admin-card">
      <h2 className="admin-card__title">TI-DSP CoE Gallery Photos ({photos.length})</h2>
      <p className="admin-lead" style={{ marginBottom: '1.5rem' }}>
        Upload the photos shown in the Gallery section of the TI-DSP Centre of Excellence differentiator page.
      </p>
