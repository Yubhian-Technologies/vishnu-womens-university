...
export interface PlacementCrtDoc {
  id: string;
  category: string;
  label: string;
  fileUrl: string;
...
const EMPTY: Omit<PlacementCrtDoc, 'id'> = {
  category: 'cdp', label: '', fileUrl: '', storagePath: '', order: 0,
};

export const CRT_CATEGORIES = [
  { key: 'cdp', label: 'CDP: Career Development Program' },
  { key: 'c-program', label: 'C-Program' },
];

export default function PlacementCrtDocsAdmin() {
  const { docs, loading } = useOrderedCollection<PlacementCrtDoc>('placementCrtDocsList', 'order');
  const [form, setForm] = useState<Omit<PlacementCrtDoc, 'id'>>(EMPTY);
  ...
  const save = async () => {
    ...
    try {
      if (editing) {
        await updateDoc(doc(db, 'placementCrtDocsList', editing), { ...form });
      } else {
        await addDoc(collection(db, 'placementCrtDocsList'), { ...form, order: form.order || docs.length + 1, createdAt: serverTimestamp() });
      }
  ...
  const startEdit = (d: PlacementCrtDoc) => {
    setEditing(d.id);
  ...
  const remove = async (id: string, storagePath?: string) => {
    ...
    try {
      ...
      await deleteDoc(doc(db, 'placementCrtDocsList', id));
    } catch (e) {
      alert(`Couldn't delete: ${(e as Error).message}`);
    }
  };

  const categoryLabel = (key: string) => CRT_CATEGORIES.find((c) => c.key === key)?.label ?? key;
    ...
    <div className="admin-section">
      <div className="admin-card">
        ...
        <p className="admin-lead" style={{ marginBottom: '1rem' }}>
          Powers the "More Details …" links under CDP: Career Development Program and C-Program on the
          Campus Recruitment &amp; Training placement page.
        </p>
        <div className="admin-form-grid">
          <div className="admin-field">
            <label htmlFor="field-category">Category *</label>
            <select id="field-category" value={form.category} onChange={(e) => set('category', e.target.value)}>
              {CRT_CATEGORIES.map((c) => <option key={c.key} value={c.key}>{c.label}</option>)}
            </select>
          </div>
          <div className="admin-field">
            <label htmlFor="field-label">Label *</label>
            <input id="field-label" value={form.label} onChange={(e) => set('label', e.target.value)} placeholder="CDP: Career Development Program Timetable" />
          </div>
          <div className="admin-field admin-field--full">
            <label>PDF File *</label>
            <FileUploader folder="vwu/placements/crt" currentUrl={form.fileUrl} onUploaded={handleFile} label="Upload PDF" />
          </div>
          <div className="admin-field">
