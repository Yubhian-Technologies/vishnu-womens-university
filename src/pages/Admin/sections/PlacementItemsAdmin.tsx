...
export default function PlacementItemsAdmin() {
  ...
  // Live "N of total" while a large batch uploads — the button would
  // otherwise just say "Uploading…" the whole time with no sign of
  // progress, which reads as stuck once there are more than a handful of
  // photos.
  ...
  const uploadBatch = async (batchItems: { name: string; makeFile: () => File }[]) => {
    setZipProgress({ done: 0, total: batchItems.length });
    let done = 0;
    let failed = 0;
    await Promise.all(batchItems.map(async ({ name, makeFile }) => {
      try {
        ...
        setZipProgress({ done, total: batchItems.length });
      }
    }));
    return { succeeded: batchItems.length - failed, failed };
  };
