...
interface TedxPhotoDoc extends WithId {
  imageUrl: string;
  storagePath: string;
  order: number;
}
...
/**
 * Photo gallery for the TEDxSVECW differentiator page — a flat, ordered
 * list of uploaded event photos (one gallery for the whole page, same
 * pattern as GSAC Photos).
 */
export default function TedxPhotosAdmin() {
  const { docs: photos, loading } = useOrderedCollection<TedxPhotoDoc>('tedxSvecwPhotos', 'order');
  const { openCrop, cropModal } = useImageCropModal(4 / 3);
  const [uploading, setUploading] = useState(false);

  const addPhoto = (file: File) => {
    setUploading(true);
    openCrop(file, 'vwu/tedxsvecw', async (result: UploadResult) => {
      try {
        await addDoc(collection(db, 'tedxSvecwPhotos'), {
          imageUrl: result.url,
  ...
  const removePhoto = async (photo: TedxPhotoDoc) => {
    if (!confirm('Remove this photo?')) return;
    try {
      await deleteDoc(doc(db, 'tedxSvecwPhotos', photo.id));
  ...
  return (
    <div className="admin-card">
      <h2 className="admin-card__title">TEDxSVECW Photos ({photos.length})</h2>
      <p className="admin-lead" style={{ marginBottom: '1.5rem' }}>
        Upload the photos shown on the TEDxSVECW differentiator page.
      </p>
